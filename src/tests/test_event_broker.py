...
class TestEventBroker:
    ...
    def test_it_notifies_event_to_a_subscription(self, mock_subscription, mock_subscription_2):
    ...
    def test_it_notifies_event_to_many_subscriptions(self, mock_subscription, mock_subscription_2):
    ...
    def test_it_notifies_event_with_extra_data_to_a_subscription(self, mock_subscription):
        waiter = EventWaiter('some event')

        EventBroker.initialize()
        EventBroker.subscribe('some event', mock_subscription)

        EventBroker.notify('some event', 'positional arg 1', 'positional arg 2', named_arg_1=1, named_arg_2=2)

        assert waiter.wait_for_event()

        mock_subscription.assert_called_once_with('positional arg 1', 'positional arg 2', named_arg_1=1, named_arg_2=2)
