...
from .temperature_sensors.temperature_sensor_descriptor import TemperatureSensorDescriptor
...
def _get_sensor_descriptors():
    ...
    for hardware_name, hardware_sensor_list in sensors.items():
        for hardware_sensor in hardware_sensor_list:
            ...
            descriptor = TemperatureSensorDescriptor(hardware_name, hardware_sensor_name)
...
def _build_monitor_descriptor(sensor_descriptor):
    sensor_id = f'{sensor_descriptor.hardware_name}-{sensor_descriptor.hardware_sensor_name}'
    monitor_type = f'temperature_sensor_{sensor_id}'
    enabled_preference_key = f'temp_monitor.{sensor_id}.enabled'
    widget_constructor = lambda: TemperatureSensorMonitorWidget(monitor_type, sensor_descriptor)
...
def execute():
    ...
    for sensor_descriptor in sensor_descriptor_list:
        ...
        Preferences.register_preference_key_default(monitor_descriptor["enabled_preference_key"], False)
